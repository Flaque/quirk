...
test("groupThoughtsByDay puts different days thoughts in different places", () => {
  ...
  expect(groups[0].thoughts[0]).toBe(mondayThought);
  expect(groups[1].thoughts[0]).toBe(tuesdayThought);
});
...
test("groupThoughtsByDay puts multiple thoughts in one day", () => {
  ...
  // Christmas
  expect(groups[0].date).toEqual(new Date("Dec 25, 2018").toDateString());
  expect(groups[0].thoughts.length).toBe(3);

  // New Years
  expect(groups[1].date).toEqual(new Date("Jan 1, 2019").toDateString());
  expect(groups[1].thoughts.length).toBe(1);
});
